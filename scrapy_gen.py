...
class ScrapyGen:
    def __init__(self, src_dir='scrapy_generator', dest_dir='test', items=['1', '2'], items_path=['path 1', 'path 2'],
                 default_src='default settings', settings_list=[], start_url='https://scrapy.generator'):
    ...
    def copy_dir(self, src):
        ...
        if src == 'default settings':
            ...
            common_files = set(File.get_files(self.dest_dir, path=False)).intersection(
                set(File.get_files(settings, path=False)))

            for com_file in common_files:
                if not File.compare(File.get_file(self.dest_dir, com_file),
                                    File.get_file(os.path.join(self.src_dir, self.default_src), com_file)):
    ...
    def items_add(self, indent=4, filename='items.py'):
        ...
        file_path = Text.get_file(self.dest_dir, filename)
        text = Text.read(file_path)
        text = Text.replace(pattern, repl, text)
        Text.write(file_path, text)
